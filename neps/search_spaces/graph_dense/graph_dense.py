...
def build(cell: Graph, num_classes: int = 10) -> Graph:
    ...
    extracted_cell = Graph()
    for u, v, data in cell.edges(data=True):
        extracted_cell.add_edge(u, v)
        extracted_cell.edges[u, v].update(data)
    extracted_cell.name = "cell"
    cell = extracted_cell

    arch = Graph()
    total_num_nodes = 20
    arch.add_nodes_from(range(1, total_num_nodes + 1))
    arch.add_edges_from([(i, i + 1) for i in range(1, total_num_nodes)])
    ...
    arch.edges[1, 2].set("op", ops.Stem(C_out=channels[0], C_in=3))

    # stage 1
    for i in range(2, 7):
        arch.edges[i, i + 1].set("op", cell.copy().set_scope("stage_1"))
    ...
    for i in range(8, 13):
        arch.edges[i, i + 1].set("op", cell.copy().set_scope("stage_2"))

    # stage 3
    arch.edges[13, 14].set(
        "op", ResNetBasicblock(C_in=channels[1], C_out=channels[2], stride=2)
    )
    for i in range(14, 19):
        arch.edges[i, i + 1].set("op", cell.copy().set_scope("stage_3"))

    # post-processing
    arch.edges[19, 20].set(
        "op",
        ops.Sequential(
...
class GraphDenseParameter:
    def __new__(
        ...
        return FunctionParameter(
            set_recursive_attribute=partial(build, num_classes=num_classes),
            old_build_api=True,
            name="graph_dense",
...
#             tree=tree,
#             terminal_to_torch_map=PRIMITIVES,
#             return_cell=True,
#         )
