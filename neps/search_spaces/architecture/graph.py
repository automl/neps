    ...
    **Use as pytorch module**
    ...
    def _assign_x_to_nodes(self, x):
        ...
        if self.input_node_idxs is None:
            ...
            assert (
                "subgraph" not in self.nodes[input_node]
            ...
            assert all(
                i in x for i in self.input_node_idxs
            ...
            if self.num_input_nodes() > len(x):
                ...
                # this can happen when there are cells with two inputs but at the
                # very first layer of the network, there is just one output (i.e. the
                # data inputed to the makro input node). Handle it and log a Info.
    ...
    def reset_weights(self, inplace: bool = False):
        ...
        graph = self if inplace else self.clone()
