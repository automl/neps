...
def unwrap_config_string(config_string: str) -> tuple[UnwrappedConfigStringPart]:
    ...
    #  generated by previous methods, the `resBlock resBlock` and `resBlock` items
    ...
    config_string = config_string.replace("resBlock resBlock", "__TMP_PLACEHOLDER___")
    config_string = config_string.replace("resBlock", "(resBlock)")
    config_string = config_string.replace("__TMP_PLACEHOLDER___", "(resBlock resBlock)")
...
def wrap_config_into_string(
    ...
    for item in unwrapped_config:
        ...
        if item.level > current_level:
            ...
            value = ")" * (current_level - item.level + 1) + " (" + item.operator
        else:
            value = ") (" + item.operator
    ...
    #  occur without wrapping parenthesis, differently from other items.
    # Wrap them appropriately in parentheses here and in the inverse process.
    result = result.replace("(resBlock resBlock)", "__TMP_PLACEHOLDER___")
    result = result.replace("(resBlock)", "resBlock")
    result = result.replace("__TMP_PLACEHOLDER___", "resBlock resBlock")
...
class ConfigString:
    ...
    def at_hierarchy_level(self, level: int) -> ConfigString:
        ...
        if level == 0:
            raise ValueError(
                f"Invalid value for `level`. Received level == 0: {level}"
            )
