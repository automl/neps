...
def get_dataset_attributes(
    ...
    def get_ave_fill_factor(Gn):
        return np.mean(
            [
                nx.number_of_edges(G) / (nx.number_of_nodes(G) * nx.number_of_nodes(G))
                for G in Gn
            ]
        )
    ...
    # graph node number
    if any(i in attr_names for i in ["ave_node_num", "min_node_num", "max_node_num"]):
    ...
    # graph edge number
    if any(i in attr_names for i in ["ave_edge_num", "min_edge_num", "max_edge_num"]):
    ...
    return OrderedDict(sorted(attrs.items(), key=lambda i: attr_names.index(i[0])))
...
def parallel_me(
    ...
    if method == "imap_unordered":
        if glbv:  # global varibles required.
            ...
            with Pool(processes=n_jobs, initializer=init_worker, initargs=glbv) as pool:
                ...
                for result in (
                    ...
                    if verbose
                    else pool.imap_unordered(func, itr, chunksize)
                ):
...
def parallel_gm(
    ...
    parallel_me(
        ...
        n_jobs=n_jobs,
        chunksize=chunksize,
        itr_desc="calculating kernel_operators",
        verbose=verbose,
    )
...
class S2VGraph:
    def __init__(
        self, g: nx.Graph, label: int, node_tags: list = None, max_node_tag: int = 5
    ):
        """
        g: a networkx graph
        ...
        if node_tags:
            assert len(node_tags) == len(
                g.nodes
            ), "mistmatch between the node_tag list and the number of nodes"
