    ...
    def parse_input(self, X):
        ...
        if not isinstance(X, collections.Iterable):
            ...
            def calculate_C(k, j, l):
                if isinstance(neighborhoods[k], Graph):
            ...
            if self._method_calling == 1:
                ...
                for l in range(1, self.L + 1):
                    ...
                    K_proj = {
                        k: np.zeros(shape=(data[k][0].shape[0], ns)) for k in range(ng)
                    }
                    ...
                    for (m, (k, j)) in enumerate(vs):
                        ...
                        for (s, (k2, j2)) in enumerate(vs):
                            if s < m:
                                K[s, m] = K[m, s] = K_proj[k2][j2, m] = K_proj[k][
                                    j, s
                                ] = self.pairwise_operation(C[s], C[m])
                            else:
                                break

                    # Compute the kernel_operators of the relations of the reference to everything else
                    for (k, j) in V[ns:]:
...
class MultiscaleLaplacian(Kernel):
    ...
    def __init__(
        self, n_jobs=None, normalize=False, verbose=False, L=3, gamma=0.01, heta=0.01
    ):
        """Initialise a `multiscale_laplacian` kernel."""
        super().__init__(n_jobs=n_jobs, normalize=normalize, verbose=verbose)
        ...
        for level in range(1, self.L + 1):
            ...
            for i in range(nx):
                ...
                for j in range(i, nx):
                    ...
                    gram_matrix_n[i, j] = self._generalized_FLG_core_(
                        qi["l"], qj["l"], extracted_gm
                    )
                ...
                for j in range(i, ny):
                    ...
                    gram_matrix_n[i, j + nx] = self._generalized_FLG_core_(
                        qi["l"], qj["l"], extracted_gm
                    )
            ...
            for i in range(ny):
                ...
                for j in range(i, nx):
                    ...
                    gram_matrix_n[idx, j] = self._generalized_FLG_core_(
                        qi["l"], qj["l"], extracted_gm
                    )
                ...
                for j in range(i, ny):
                    ...
                    gram_matrix_n[idx, j + nx] = self._generalized_FLG_core_(
                        qi["l"], qj["l"], extracted_gm
                    )
