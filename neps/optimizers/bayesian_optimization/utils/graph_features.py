...
class FeatureExtractor:
    ...
    def __init__(self, g: nx.Graph, node_attr_name="op_name", s="input", t="output"):
        ...
        for n in range(g.number_of_nodes()):
            assert node_attr_name in list(dict(g.nodes[n]).keys()), (
                node_attr_name + " is not found in " + str(g.nodes[n])
            )
    ...
    def average_undirected_degree(self):
        return sum(dict(self.undirected_g.degree).values()) / (
            self.undirected_g.number_of_nodes() + 0.0
        )
