...
class Interpreter:
    def __init__(self, X=None, y=None, h=1, gp=None, thres=25, occur_thres=10):
        ...
        self.n201 = ["nor_conv_3x3", "nor_conv_1x1", "avg_pool_3x3", "input", "output"]

        self.feat_graphlets = [
            encoding_to_nx(fe, color_map=self.color_map) for fe in self.feat_list
        ]
        up = np.percentile(self.grad_, 100.0 - thres)
    ...
    def set_threshold(self, threshold):
        ...
        up = np.percentile(self.grad_, 100.0 - threshold)
    ...
    def explain(self, X_s, y_s, percentile):
        ...
        for i in self.good_idx:
            if embedding[int(i)] != 0:
                good_match.append(int(i))
        for i in self.bad_idx:
            if embedding[int(i)] != 0:
                bad_match.append(int(i))
            ...
            for i in match_set:
                ...
                if len(f) == 1:  ### h = 0 features
                    ...
                    gm = isomorphism.DiGraphMatcher(
                        X_s, f, node_match=lambda x, y: x["op_name"] == y["op_name"]
                    )
        ...
        if percentile > 80:
            plt.title(
                "Valid Acc.: %.2f %%. Percentile: %.2f"
                % ((1.0 - np.exp(-y_s)) * 100.0, percentile),
                color="green",
            )
        elif percentile < 20:
            plt.title(
        ...
        if len(bad_match):
            ...
            if len(sn):
                nodes = nx.draw_networkx_nodes(
                    sn,
                    pos,
                    node_color=[X_s.nodes[i]["color"] for i in sn],
                    linewidths=3,
                )
        ...
        color = [
            data["color"] for v, data in X_s.nodes(data=True) if v not in special_nodes
        ]
    ...
    def get_legend(self, select=None):
        ...
        lines = [
            Line2D([0], [0], color=c, linewidth=3, linestyle="None", marker="o")
            for c in colors
        ]
...
if __name__ == "__main__":
    ...
    from ..benchmarks.nas.nasbench import NAS101Cifar10
    from ..initial_design.generate_test_graphs import random_sampling
    ...
    if isinstance(e, NAS101Cifar10):
        ...
        g = 1.0 - np.exp(-np.array(g).flatten())
        b = 1.0 - np.exp(-np.array(b).flatten())
        y = 1.0 - np.exp(-y_s_np)
        data = [g, y, b]
        ...
        sns.kdeplot(
            y, shade=True, label="all", color="blue", clip=(y.min(), y.max()), alpha=0.2
        )
        sns.kdeplot(
            b, shade=True, label="bad", color="red", clip=(b.min(), b.max()), alpha=0.2
        )
        ...
        for task in ["cifar10-valid", "cifar100", "ImageNet16-120"]:
            ...
            plt.legend()
            plt.savefig(
                "valid_searched_on" + ori_task + "_on" + task + ".png",
                bbox_inches="tight",
            )
            # plt.show()
