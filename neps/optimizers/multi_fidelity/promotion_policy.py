...
class SyncPromotionPolicy(PromotionPolicy):
    ...
    def retrieve_promotions(self) -> dict:
        ...
        self.rung_promotions = {rung: [] for rung in self.rung_members.keys()}
        total_rung_evals = 0
        for rung in reversed(sorted(self.config_map.keys())):
            total_rung_evals += len(self.rung_members[rung])
            if total_rung_evals >= self.config_map[rung] and np.isnan(self.rung_members_performance[rung]).sum():
            ...
            if rung == self.max_rung:
                ...
                continue
            if total_rung_evals >= self.config_map[rung] and np.isnan(self.rung_members_performance[rung]).sum() == 0:
