...
class MFObservedData:
    ...
    def __init__(
        ...
        learning_curve_col: str | None = None,
        auxiliary_cols: list[str] | None = None,
    ):
        self.config_col = self.default_config_col if config_col is None else config_col
        self.perf_col = self.default_perf_col if perf_col is None else perf_col

        self.config_idx = self.default_config_idx if config_id is None else config_id
        self.budget_idx = self.default_budget_idx if budget_id is None else budget_id

        self.lc_col_name = learning_curve_col

        auxiliary_cols = [] if auxiliary_cols is None else auxiliary_cols

        self.index_names = [self.config_idx, self.budget_idx]
        col_names = [self.config_col, self.perf_col, self.lc_col_name] + auxiliary_cols
        self.columns = [col_name for col_name in col_names if col_name is not None]

        index = pd.MultiIndex.from_tuples([], names=self.index_names)

        self.df = pd.DataFrame([], columns=self.columns, index=index)

        self.mutable_columns = [self.config_col, self.lc_col_name]
    ...
    @staticmethod
    def __validate_index(index_list):
        """Extends single indices to multi-index case"""
        if all([isinstance(idx, int) for idx in index_list]):
            index_list = list(zip(index_list, [0] * len(index_list)))
        return index_list
    ...
    def add_data(
        ...
        index_list = self.__validate_index(index_list)
    ...
    def get_incumbents_for_budgets(self, maximize: bool = False):
        ...
        if maximize:
            config_ids = trajectories.idxmax(axis=0)
        else:
            config_ids = trajectories.idxmin(axis=0)
    ...
    def get_best_performance_for_each_budget(self, maximize: bool = False):
        ...
        trajectories = self.get_trajectories()
        if maximize:
            performance = trajectories.max(axis=0)
        else:
            performance = trajectories.min(axis=0)
    ...
    def get_best_learning_curve_id(self, maximize: bool = False):
        ...
        trajectories = self.get_trajectories()
        if maximize:
            return trajectories.max(axis=1).idxmax()
        else:
            return trajectories.min(axis=1).idxmin()

    def get_best_seen_performance(self, maximize: bool = False):
        trajectories = self.get_trajectories()
        if maximize:
            return trajectories.max(axis=1).max()
        else:
            return trajectories.min(axis=1).min()

    def add_budget_column(self):
        pass
        # budget_column = self.df.index.get_level_values(1)
        # self.df[self.budget_idx] = budget_column
    ...
    def copy_df(self, df: pd.DataFrame | None = None):
        """
        Use this function to copy df if you are going to
        perform some operations on its elements.

        DataFrames are not meant for mutable data-types,
        nevertheless we do put mutable SearchSpace objects into the config_col of the DF
        In order not to change the values stored objects in the DF we deepcopy all
        mutable columns here.

        self.mutable_columns must keep track of
        the mutable columns at all times.
        """
        if df is None:
            df = self.df
        new_df = pd.DataFrame()
        new_df.index = df.index.copy(deep=True)

        for column in df.columns:
            if column in self.mutable_columns:
                new_column = [deepcopy(value) for value in df[column].values]
    ...
    def reduce_to_max_seen_budgets(self):
        ...
        budget_column = self.df.index.get_level_values(1)
        combined_df = self.df.copy(deep=True)
        combined_df[self.budget_idx] = budget_column
        # combined_df = self.copy_df(df=combined_df)
    ...
    def extract_learning_curve(
        ...
        if self.lc_col_name in self.df.columns:
            ...
            trajectories = self.get_trajectories()
            lc = trajectories.loc[config_id, :budget_id].values.flatten().tolist()
        return deepcopy(lc)
...
if __name__ == "__main__":
    ...
    def multi_index_single():
        ...
        data.add_data(["conf1", 0.8], index=(1, 0))
        print(data.df)

        data.update_data({"perf": [1.8], "budget_col": [5]}, index=(0, 0))
        print(data.df)

    def single_index_parallel():
        data = MFObservedData(
            config_id="config_id",
            budget_id="budget_id",
            config_col="config",
        ...
        data.add_data(
            [["conf1", 0.5], ["conf2", 0.10]],
            index=[(2), (3)],
        )

        print(data.df)

        data.update_data({"perf": [1.8, 1.5]}, index=[(1), (0)])
        print(data.df)

    """
    Here are a few examples of how to manage data with this class:
    """
    single_index_parallel()
    multi_index_single()
    single_index_parallel()
