...
from .priorband_template import pipeline_space, evaluate_pipeline
...
def ifbo_run_pipeline(
    ...
    **config,  # The hyperparameters to be used in the pipeline
    ...
    warn("ifbo_run_pipeline is deprecated, use ifbo_evaluate_pipeline instead", DeprecationWarning)
    return ifbo_evaluate_pipeline(pipeline_directory, previous_pipeline_directory, **config)

def ifbo_evaluate_pipeline(
    pipeline_directory,  # The directory where the config is saved
    previous_pipeline_directory,  # The directory of the config's immediate lower fidelity
    **config,  # The hyperparameters to be used in the pipeline
) -> dict | float:
    result_dict = evaluate_pipeline(
