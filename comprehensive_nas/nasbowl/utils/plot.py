...
def make_incumbent_plot(strategy_results_dict, strategies, save_folder_name, benchmark_name, title_type, experiment_name=None):
...
def plot_losses(fig, ax, axins, incumbent_trajectories, regret=True,
    ...
    for i, (m, tr) in enumerate(incumbent_trajectories.items()):
        ...
        ax.errorbar(tr['time_stamps'], mean, sem, uplims=True, lolims=True,
                    errorevery=errorevery, color=color_marker_dict[m]['color'], alpha=0.4)

        ax.plot(tr['time_stamps'], mean,
                label=labels.get(m, m), color=color_marker_dict[m]['color'], linewidth=linewidth,
                marker=color_marker_dict[m]['marker'], markersize=marker_size, markevery=(0.1, 0.1))
