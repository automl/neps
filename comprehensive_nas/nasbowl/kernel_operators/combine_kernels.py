from nasbowl.kernel_operators.weisfilerlehman import GraphKernels
from nasbowl.kernel_operators.vectorial_kernels import Stationary, HammingKernel
import torch
import logging
...
class CombineKernel:
    ...
    def fit_transform(self, weights, gr1: list, x1: list,
                      normalize=True,
        ...
        for i, k in enumerate(self.kernels):
            if isinstance(k, GraphKernels) and None not in gr1:
                update_val = weights[i] * k.fit_transform([g[i] for g in gr1] if isinstance(gr1[0], tuple)
                                                        else gr1,
                ...
                #     continue
                update_val = (weights[i] * k.fit_transform(x1, rebuild_model=rebuild_model,
                                                           save_gram_matrix=save_gram_matrix)).double()

            else:
                raise NotImplementedError(" For now, only the Stationary custom built kernel_operators are supported!")
    ...
    def transform(self, weights, gr: list, x=None, feature_lengthscale=None):
        ...
        K = torch.zeros(size, self._gram.shape[0]) if self.combined_by == 'sum' \
            else torch.ones(size, self._gram.shape[0])

        for i, k in enumerate(self.kernels):
            if isinstance(k, GraphKernels) and None not in gr:
                update_val = weights[i] * k.transform([g[i] for g in gr] if isinstance(gr, tuple) else gr)
