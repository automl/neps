...
from nasbowl.benchmarks.nas.nb_configfiles.api.nas_201_api import NASBench201API as API
import ConfigSpace
import random
...
class NASBench201(AbstractBenchmark):
    ...
    def __init__(self, data_dir, task='cifar10-valid', log_scale=True, negative=True,
        ...
        self.api = API(os.path.join(data_dir, 'NAS-Bench-201-v1_0-e61699.pth'), verbose=False)
        ...
        super(NASBench201, self).__init__(dim=None, optimum_location=best_test_arch_index, optimal_val=best_test_err,
                                          bounds=None)
    ...
    def _retrieve(self, G, hps, which='eval'):
        ...
        if G is None:
            ...
            op_node_labeling = [op_labeling["edge_%d" % i] for i in range(len(op_labeling.keys()))]
        ...
        try:
            ...
            acc_results = self.api.query_by_index(arch_index, self.task, hp=self.hp,)
    ...
    def eval(self, G, hps=None, n_repeat=1):
        ...
        return np.mean(np.array([self._retrieve(G, hps, 'eval') for _ in range(n_repeat)]))

    def test(self, G, hps, n_repeat=1):
        return np.mean(np.array([self._retrieve(G, hps, 'test') for _ in range(n_repeat)]))
...
if __name__ == '__main__':
    ...
    nascifar10 = NASBench201(data_dir=output_path, task='cifar10-valid', seed=0, negative=False, log_scale=False)
    f = nascifar10.eval
    result = f(G)
