...
from nasbowl.benchmarks.nas.nasbench201 import *
from nasbowl.benchmarks.hpo.branin2 import *
from nasbowl.benchmarks.hpo.hartmann3 import *
from nasbowl.benchmarks.hpo.hartmann6 import *
from nasbowl.benchmarks.hpo.counting_ones import *
...
class Sampler:
    def __init__(self, args, objective):
        ...
        self.dataset = args.dataset
        if self.dataset in ['nasbench201', 'nasbench301', 'branin2', 'hartmann3', 'hartmann6', 'counting_ones']:
            self.default_config = objective.get_config_space().get_default_configuration().get_dictionary()
        else:
    ...
    def sample(self, pool_size):
                ...
                for key, item in config_dict.items():
                    ...
                    if 'hyperparam' in key:
                        ...
                        if type(hp) == ConfigSpace.OrdinalHyperparameter:
                            ...
                            if hp.log:
                                hyperparameters[i] = np.log(val / bounds[0]) / np.log(bounds[1] / bounds[0])
                            else:
                                hyperparameters[i] = (config[key] - bounds[0]) / (bounds[1] - bounds[0])
                        i += 1
        ...
        return pool_graphs if self.optimize_arch else [None] * pool_size,\
