...
class StatisticsTracker(object):
    ...
    def calculate_incumbent(self, x, y, next_y):
        ...
        incumbent_value = np.exp(-y[self.n_init:][best_idx]).item() if self.log else -y[self.n_init:][best_idx].item()
    ...
    def print(self, x, y, next_y, train_details):
        ...
        values = [str(self.iteration), str(self.last_func_eval), str(self.incumbent_values[-1]),
                  str(self.end_time - self.start_time), str(self.cum_train_times[-1])]
    ...
    def save_results(self, incumbents_config):
        ...
        if self.save_path is not None:
            with open(os.path.join(self.save_path, "{}:{}.json".format(self.max_iters, self.seed)), "w") as f:
