...
class NASBenchMetaAPI(metaclass=abc.ABCMeta):
    @abc.abstractmethod
    def __init__(
        self, file_path_or_dict: Optional[Union[Text, Dict]] = None, verbose: bool = True
    ):
    ...
    def get_total_epoch(self, dataset=None):
        ...
        if dataset is None:
            ...
            for xdata, x_seeds in self.dataset_seed.items():
                epochss += [
                    self.all_results[(xdata, seed)].get_total_epoch() for seed in x_seeds
                ]
        ...
        for key, value in self.__dict__.items():
            if key == "all_results":  # contain the class of ResultsCount
                ...
                for _k, _v in value.items():
                    assert isinstance(
                        _v, ResultsCount
                    ), "invalid type of value for {:}/{:} : {:}".format(key, _k, type(_v))
...
class ResultsCount(object):
    ...
    def get_times(self):
        ...
        for name in self.eval_names:
            try:
                xtimes = [
                    self.eval_times["{:}@{:}".format(name, i)] for i in range(self.epochs)
                ]
