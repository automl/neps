...
class SAGEConvolutions(nn.Module):
    def __init__(
        ...
        if lin is True:
            self.lin = nn.Linear(
                (NUM_SAGE_LAYERS - 1) * hidden_channels + out_channels, out_channels
            )
        else:
...
class DiffPool(nn.Module):
    ...
    def __init__(self, dim_features, dim_target, model_config):
        ...
        gnn_dim_hidden = model_config[
            "gnn_hidden_dimensions"
        ]  # embedding size of first 2 SAGE convolutions
        dim_embedding = model_config[
            "dim_embedding"
        ]  # embedding size of 3rd SAGE convolutions (eq. 5, dim of Z)
        dim_embedding_MLP = model_config[
            "dim_embedding_MLP"
        ]  # hidden neurons of last 2 MLP layers
        ...
        for i in range(num_diffpool_layers):
            diffpool_layer = DiffPoolLayer(
                gnn_dim_input, gnn_dim_hidden, dim_embedding, no_new_clusters
            )
        ...
        self.final_embed = SAGEConvolutions(
            gnn_embed_dim_output, gnn_dim_hidden, dim_embedding, lin=False
        )
    ...
    def forward(self, graph_batch):
        ...
        for i in range(self.num_diffpool_layers):
            ...
            x, adj, l, e = self.diffpool_layers[i](
                x, adj, mask
            )  # x has shape (batch, MAX_no_nodes, feature_size)
