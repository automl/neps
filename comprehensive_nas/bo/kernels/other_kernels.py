...
class RandomWalk(GraphKernels):
    ...
    def __init__(
        self, lambda_=0.001, fast: bool = True, node_label: str = "op_name", **kwargs
    ):
        ...
        if not self.fast:
            logging.warning(
                ".fast flag has been turned off, and O(n^6) complexity is incurred in computing the exact"
                "random walk kernel!"
            )
        self.kern = _RWL(
            lamda=lambda_, method_type="fast" if fast else "baseline", normalize=True
        )  # For use in Gaussian Process, normalize is required
        self.__name__ = "RandomWalk"
        self._gram, self._train, self._train_transformed = None, None, None
...
class ShortestPath(RandomWalk):
    def __init__(self, sp_algo="auto", node_label="op_name", **kwargs):
        ...
        self.kern = _SPA(
            algorithm_type=self.sp_algo, normalize=True, with_labels=True
        )  # For use in Gaussian Process, normalize is required
        self.__name__ = "RandomWalk"
